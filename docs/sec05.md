...
-   Docker を利用して起動すると PostgreSQL のコンテナを利用する
    -   なおデータは `db_data/` というフォルダ以下永続化される．このフォルダのオーナーおよびグループは，コンテナ内で定義された `postgres` および `root` になるので，ユーザ権限では削除できない．削除する場合は `sudo` などを利用すること．
-   Docker を利用しない場合(WSLのターミナルで `uvicorn api.main:app --host 0.0.0.0` 等でサーバを起動する場合)は [SQLite](https://sqlite.org/) を利用する(`data.json` の代わりに `app.db` にデータが保持される)
