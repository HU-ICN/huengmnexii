...
[sec04.zip](sec04.zip)をダウンロードし，`sec04.zip`があるディレクトリにて `unzip sec04.zip` を実行すれば展開される(今いるディレクトリに展開されるので注意すること; 個別にダウンロードしてもよい)．
...
実際に「REST API」と呼ばれているものの多くは，厳密なRESTの定義(特にハイパーメディア制約(HATEOAS))を満たしておらず，単にHTTPを用いたCRUD型のAPIを指している場合も多い．
